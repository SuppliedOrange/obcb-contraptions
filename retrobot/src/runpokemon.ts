...
const game = "./games/Bomberman (U).nes";
const startIndex = 800412;
// 1048576
const page = 4;
// 5
const use_id: string | null = "legendofzeldagb";
const gameHorizontal =true;

import { InputState } from './util';
import { CoreType, emulate } from './emulate';
import { setGameInfo, GameInfo, InputAssist, InputAssistSpeed, DirectionPress } from './gameInfo';
...
async function setup() {
    ...
    class ButtonState {
        ...
        async loadButtons() {
                this.up = {
                    ...
                    hitboxRow: (!gameHorizontal) ? 48 : 15,
                ...
                this.down = {
                    ...
                    hitboxRow: (!gameHorizontal) ? 63 : 15,
                    hitboxHeight: (!gameHorizontal) ? 5 : 8,
                    hitboxWidth: (!gameHorizontal) ? 10 : 7
                };
                ...
                this.left = {
                    ...
                    hitboxRow: (!gameHorizontal) ? 55 : 18,
                    hitboxHeight: (!gameHorizontal) ? 7 : 10,
                    hitboxWidth: (!gameHorizontal) ? 5 : 5
                };
                ...
                this.a = {
                    ...
                    hitboxRow: (!gameHorizontal) ? 60 : 19,
                ...
                this.select = {
                    ...
                    hitboxRow: (!gameHorizontal) ? 43 : 24,
                    hitboxHeight: (!gameHorizontal) ? 3 : 5,
                    hitboxWidth: (!gameHorizontal) ? 5 : 3
                };
    ...
    fs.writeFileSync(gameFile, gameBuffer);

    const info: GameInfo = {
        game,
        coreType,
        inputAssist: InputAssist.Wait,
    ...
    async function triggerUpdate(emulationData?: EmulationData) {
        ...
        const stateFile = path.join(data, 'state-pokemon.sav');

        console.log('Got output');

        fs.writeFileSync(stateFile, state);
        fs.writeFileSync("output-pokemon.gif", recording);
    ...
    async function convertToBlackAndWhite(imagePath: string | Buffer, targetWidth: number, startIndex=0, threshold = 70) {
    ...
    async function renderButtons( gameScreenPath: string, messagePath: string ) {
        ...
        if (gameHorizontal) {
            ...
            const sourceHeight = 300; // Height of the cropped area on the source image (less than original height)

            // Draw the cropped and adjusted game screen onto the canvas
            ctx.drawImage(
                gameScreen, 
                0,             // x coordinate on the source image
                0,             // y coordinate on the source image
                480,           // width of the area to crop from the source image
                300,           // height of the area to crop from the source image
                -20,           // x coordinate on the canvas
                -15,           // y coordinate on the canvas
                510,           // width to draw on the canvas (scaled)
                340            // height to draw on the canvas (scaled)
            );
            */
            ctx.drawImage(gameScreen, 0, 0, 480, 320);
            ctx.drawImage(message, 0, 572, 480, 140);

            // up triangle
            let upTriangle = buttonState.getImage(buttonState.up);
            ctx.save();
            ctx.translate(105, 400);
            ctx.rotate(0); // No rotation, facing up
            ctx.drawImage(upTriangle, -upTriangle.width / 2, -upTriangle.height / 2);
            ctx.restore();
            ...
            // b
            let bCircle = buttonState.getImage(buttonState.b);
            ctx.drawImage(bCircle, 370, 360, bCircle.width, bCircle.height);

            // a
            let aCircle = buttonState.getImage(buttonState.a);
            ctx.drawImage(aCircle, 320, 460, aCircle.width, aCircle.height);

            // Start button
            let startButton = buttonState.getImage(buttonState.start);
            ctx.drawImage(startButton, 150, 315, startButton.width - 20, startButton.height - 20);

            // Select button
            let selectButton = buttonState.getImage(buttonState.select);
            ctx.drawImage(selectButton, 250, 315, selectButton.width - 20, selectButton.height - 20);
        }

        if (gameHorizontal) sharp(canvas.toBuffer()).rotate(90).toFile('rendered-pokemon.png');
        else fs.writeFile('rendered-pokemon.png', canvas.toBuffer(), (err) => {});
    ...
    async function drawNewMessage(name: string, framePath: string = 'final-frame-pokemon.png') {

        let messagePath = `../images/${name}_screen.jpg`;
        let buttonOverlayImage = await renderButtons(framePath, messagePath);
        let { binaryPixels } = await convertToBlackAndWhite( buttonOverlayImage, 60, startIndex );
    ...
    async function loadGif(activatedButton?: ButtonStateData) {
        ...
        let outputFolder = "./exported_frames-pokemon"
        let messagePath = "../images/wait_screen.jpg"

        const results = await extractFrames({
            input: 'output-pokemon.gif',
        ...
        for (let index = 0; index < framesRendered; index++ ) {
            ...
            let finalFramePath = 'final-frame-pokemon.png';
            ...
            // Draw the iamge
            // await navigateAndWait(client,page)
            await drawImage( binaryPixels,  true )
            currentBinaryPixels = binaryPixels;
            await new Promise(r => setTimeout(r, 50));
            
            firstRunAfterRedraw += 2;
    ...
    async function handleClick( pixelIndices: number[] ) {

        if (pixelIndices.length) console.log(pixelIndices[0]);

        let toToggle: null | ButtonStateData = null;

        for (const pixel of pixelIndices) {
            if (toToggle) break;
            for (const button of buttonState.buttons) {
                if (toToggle) break;
                if (isPixelInHitbox(pixel, button.hitboxRow, button.hitboxColumn, button.hitboxWidth, button.hitboxHeight)) {
                    toToggle = button;
                    console.log(`${toToggle.name} was clicked`);
                }
            }
        }   
        ...
        toToggle.isActivated = true;

        await drawNewMessage( 'run' );
        
        let playerInputs: InputState[] = [];

        let multiplier = 2;
        playerInputs = range(0, multiplier).map(() => parseInput(toToggle.name));

        if (playerInputs.length > 0) {

            let game = fs.readFileSync(path.resolve(info.game))
            let oldState = fs.readFileSync(path.resolve('data', id, 'state-pokemon.sav'));
            await triggerUpdate({
                pool: pool,
    ...
    while (true) {
        ...
        try {
            ...
            // let clicks = await drawImage(currentBinaryPixels);
            let clicks = await drawImage(currentBinaryPixels);
            if (firstRunAfterRedraw > 0) {
                firstRunAfterRedraw -= 1;
                continue
            }
            else if (clicks) await handleClick(clicks)
        }
        catch (error) {
            console.log(error);
        }
