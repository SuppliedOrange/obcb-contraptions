...
const game = "./games/Tetris.nes";
const startIndex = 791292;
// 1048576
const page = 4;
// 5
const use_id: string | null = "tetris-save";
...
async function setup() {
    ...
    const info: GameInfo = {
        ...
        inputAssistSpeed: InputAssistSpeed.Slow,
    ...
    async function triggerUpdate(emulationData?: EmulationData) {
        ...
        fs.writeFileSync("output-tetris.gif", recording, {flag: "w+"});
    ...
    async function renderButtons( gameScreenPath: string, messagePath: string ) {
        ...
        if (gameHorizontal) {
            ...
            ctx.drawImage(message, 0, 972, 480, 140);

            // up triangle
            let upTriangle = buttonState.getImage(buttonState.up);
            ctx.save();
            ctx.translate(105, 800);
            ctx.rotate(0); // No rotation, facing up
            ctx.drawImage(upTriangle, -upTriangle.width / 2, -upTriangle.height / 2);
            ctx.restore();
            ...
            // b
            let bCircle = buttonState.getImage(buttonState.b);
            ctx.drawImage(bCircle, 370, 760, bCircle.width, bCircle.height);

            // a
            let aCircle = buttonState.getImage(buttonState.a);
            ctx.drawImage(aCircle, 320, 860, aCircle.width, aCircle.height);

            // Start button
            let startButton = buttonState.getImage(buttonState.start);
            ctx.drawImage(startButton, 150, 715, startButton.width - 20, startButton.height - 20);

            // Select button
            let selectButton = buttonState.getImage(buttonState.select);
            ctx.drawImage(selectButton, 250, 715, selectButton.width - 20, selectButton.height - 20);
        }

        if (gameHorizontal) sharp(canvas.toBuffer()).rotate(90).toFile('rendered-tetris.png');
        else fs.writeFile('rendered-tetris.png', canvas.toBuffer(), (err) => {}, );
    ...
    async function drawNewMessage(name: string, framePath: string = 'final-frame-tetris.png') {

        let messagePath = `../images/${name}_screen.jpg`;
        let buttonOverlayImage = await renderButtons(framePath, messagePath);
        let { binaryPixels } = await convertToBlackAndWhite( buttonOverlayImage, 60, startIndex, 130 );
    ...
    async function loadGif(activatedButton?: ButtonStateData) {
        ...
        let outputFolder = "./exported_frames-tetris"
        let messagePath = "../images/wait_screen.jpg"

        const results = await extractFrames({
            input: 'output-tetris.gif',
        ...
        for (let index = 0; index < framesRendered; index++ ) {
            ...
            let finalFramePath = 'final-frame-tetris.png';
    ...
    while (true) {
        ...
        try {
            ...
            if (firstRunAfterRedraw) {
                firstRunAfterRedraw = false;
            }
